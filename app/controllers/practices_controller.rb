class PracticesController < ApplicationController
  ...
  def show
    @page_url = params[:page_url] || 'industry_practice/banking'
    @class = @page_url.split('/')[1]
    @allpages = Page.where(:activate => true, :locale => I18n.locale).order("display_order")
    @main = Page.where(:activate => true, :page_url => 'our_practices', :locale => I18n.locale).limit(1).first
    ...
    @industry_practices = Page.all(:conditions => ["page_url LIKE ? and locale = ?", 'industry_practice%', I18n.locale ], :order => 'display_order')
